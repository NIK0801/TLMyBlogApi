    ...
    public class PostRepository : IRepository<Post>
    {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            posts.Add(new Post
                            {
                                ...
                                IsPublished = Convert.ToInt32(reader["is_published"]),
        ...
        public int Create(Post post)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                using (SqlCommand cmd = connection.CreateCommand())
                {
                    cmd.CommandText = "INSERT INTO [post] (title, content, category_id) VALUES (@title, @content, @category_id)";
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            return new Post
                            {
                                ...
                                Title = Convert.ToString(reader["title"]),
                                Content = Convert.ToString(reader["content"]),
                                IsPublished = Convert.ToByte(reader["is_published"]),
                                CategoryId = Convert.ToInt32(reader["category_id"])
                            };
                        }
                        else
        ...
        public int Update(Post post)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                using (SqlCommand cmd = connection.CreateCommand())
                {
                    cmd.CommandText = @"UPDATE [post] SET [title] = @title, [content] = @content, [category_id] = @category_id
                        WHERE [id] = @id";
                    cmd.Parameters.Add("@title", SqlDbType.NVarChar).Value = post.Title;
                    cmd.Parameters.Add("@content", SqlDbType.NVarChar).Value = post.Content;
                    cmd.Parameters.Add("@category_id", SqlDbType.NVarChar).Value = post.CategoryId;
